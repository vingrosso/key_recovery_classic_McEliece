...
uint8_t store_support(uint8_t* support, uint8_t len)
{
  /* The 34-byte pk array comprises the following fields:
     - 2 byte   : offset
     - 1 byte   : number of values
     - 30 bytes : values to store in the support L
     At most 34 bytes can be sent, expect serial problems otherwise
  */
  uint8_t OK[30] = {0};
  uint8_t offset_1 = support[0];
  uint8_t offset_2 = support[1];
  uint8_t nb_values = support[2]/sizeof(gf);
  for (int val = 0; val<nb_values; val++) {
  	L[15*offset_1+3840*offset_2+val] = load_gf(support+3+2*val);
  	store_gf(OK + 2*val, L[15*offset_1+3840*offset_2+val]);
  }
  #ifdef STREAM_MODE
  simpleserial_put('L', 30, OK);
  #else
...
int main(void)
{
  ...
  #ifdef STREAM_MODE
	simpleserial_addcmd('L', 3+30, store_support);	                      //Store the support L
  #else
